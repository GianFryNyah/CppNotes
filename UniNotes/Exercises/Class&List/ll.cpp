...
bool ll::remove_last_rec(int e, cell* curr, cell* previous){
    ...
    if(!removed && curr->value == e){
        ...
        if(previous == nullptr)
            ...
            previous->next = curr->next; // connecting previous cell with the cell next to the cell to delete
        delete curr; // finally deleting the cell
        return true;
    }
    return removed;
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Return true if the two lists have the same length and the same values in
...
bool ll::operator==(const ll& other) const{
    ...
    // cell* pc1 = (*this).head; cell* pc2 = other.head;
    // return equal(pc1, pc2);
    // RECURSIVE IMPLEMENTATION
    cell* pc1 = (*this).head; cell* pc2 = other.head;
    return equal_rec(pc1, pc2);
}

bool ll::equal(cell* head, cell* head_other) const{
    bool flag = true;
    while(head != nullptr && head_other != nullptr){
        if(head->value != head_other->value){
            flag = false;
        }
        head = head->next; head_other = head_other->next;
    }

    if(!(head == nullptr && head_other == nullptr)){
        flag = false;
    }
...
bool ll::equal_rec(cell* curr, cell* curr_other) const{
    ...
    bool flag = equal_rec(curr->next, curr_other->next);

    if((curr->value == curr_other->value) && flag){flag = true;}
    else{flag = false;}
...
ll& ll::operator=(const ll& other){
    if(this != &other){ // this e' il PUNTATORE, *this e' l'OGGETTO ; this != &other mi dice: sono diversi i DUE PUNTATORI?
        cell* pc;
        while(head!=nullptr){
            pc = head;
            head = head->next;
...
void ll::remove_all(int e){
    cell* prev = nullptr; cell* curr; cell*next;
    curr = head;
    while((curr->next) != nullptr){
        if(curr->value == e){
            // delete curr and link prev with next
            cell* tmp = curr;
            if(prev == nullptr){
                head = curr->next;
                ...
                prev->next = curr->next;
                (prev->next)->value = (curr->next)->value;
                delete tmp;
            }
            prev = curr->next;
            curr = (curr->next)->next;
...
void ll::flip_rec(cell*& curr){ //?? doesn't work
    if(!curr) return;
    if(!curr->next) return;
...
int main(){
    ...
    cout << "list f: ";
    f.append(6); f.prepend(6);
    f.remove_all(6);
    f.print();
    cout << "the sum of f' elements is: " << f.sum() << endl;
    cout << "flipped list f: ";
    f.flip();
    f.print();
